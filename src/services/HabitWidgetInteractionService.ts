...
.dashflow-habit-row-head{display:flex;align-items:center;justify-content:space-between;gap:10px}
.dashflow-habit-name{appearance:none;border:0;background:transparent;padding:0;color:var(--text-normal);font-weight:600;text-align:left;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;cursor:pointer}
.dashflow-habit-name:hover{color:var(--interactive-accent)}
.dashflow-habit-streak{font-size:10px;color:var(--text-muted);white-space:nowrap}
.dashflow-habit-history{display:grid;grid-auto-flow:column;grid-auto-columns:minmax(10px,1fr);gap:4px;align-items:center}
.dashflow-habit-day{height:12px;border-radius:3px;background:var(--background-modifier-border);position:relative}
...
.dashflow-habit-meta{display:flex;align-items:center;justify-content:space-between;gap:10px}
...
.dashflow-habit-check{font-size:10px;padding:4px 8px;border-radius:7px;white-space:nowrap}
...
@media(max-width:900px){.dashflow-habit-row{padding:8px}.dashflow-habit-history{gap:3px}.dashflow-habit-day{height:11px}}
`;
...
export class HabitWidgetInteractionService {
  ...
  private renderHabits(body: HTMLElement, widget: WidgetInstance): void {
    ...
    add.textContent = "+";
    add.title = "新建习惯";
    add.setAttribute("aria-label", "新建习惯");
    ...
    if (habits.length === 0) {
      ...
      empty.textContent = "还没有习惯。点击右上角 ＋ 创建第一个长期目标。";
  ...
  private renderHabit(habit: Habit, historyDays: number, config: HabitsWidgetConfig): HTMLElement {
    ...
    row.className = `dashflow-habit-row${habit.status === "paused" ? " is-paused" : ""}`;
    ...
    streak.textContent = `🔥 ${habitCurrentStreak(habit, today)}`;
    ...
    for (const point of habitHistory(habit, historyDays, today)) {
      ...
      cell.title = `${point.date} · ${point.scheduled ? (point.completed ? "已完成" : "未完成") : "休息日"}`;
    ...
    if (habit.status === "paused") {
      ...
      check.textContent = isDone ? "✓ 已打卡" : "○ 打卡";
    ...
    meta.append(progressWrap, check);
