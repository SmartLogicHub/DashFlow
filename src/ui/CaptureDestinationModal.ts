...
export class CaptureDestinationModal extends Modal {
  ...
  onOpen(): void {
    ...
    contentEl.createDiv({ cls: "dashflow-modal-eyebrow", text: "QUICK CAPTURE" });
    ...
    options.append(
      this.option("inbox", "inbox", "DashFlow Inbox", "作为待整理任务写入 Inbox"),
      this.option("daily-note", "notebook-pen", "今天的 Daily Note", "作为普通笔记条目追加，保留 #标签 与 [[双链]]"),
    );
  }
